...
//
//func newHookpointOld(pkgPath string, funcDecl *dst.FuncDecl, descriptorTypeIdent string, descriptorValueInitializer hookDescriptorValueInitializer) *hookpoint {
...
func newHookpoint(pkgPath string, funcDecl *dst.FuncDecl, descriptorTypeIdent string, descriptorValueInitializer hookDescriptorValueInitializer) *hookpoint {
	...
	epilogFuncType, _ := newSqreenEpilogFuncType(funcDecl.Type)
	prologFuncType, _ := newSqreenPrologFuncType(funcDecl, epilogFuncType)
	...
	var instrumentationStmt dst.Stmt
	switch funcDecl.Name.String() {
	case "execute":
		args := newRuntimeExecuteInstrumentationArgs(funcDecl)
		instrumentationStmt = newInstrumentationStmt("_ddog_runtime_execute", args)
	//case "Goexit":
	//	instrumentationStmt = newInstrumentationStmt("_ddog_runtime_goexit", prologCallArgs)
	}
...
func newRuntimeExecuteInstrumentationArgs(funcDecl *dst.FuncDecl) []dst.Expr {
	args := []dst.Expr{
		&dst.SelectorExpr{
			X: &dst.Ident{
				Name: "gp",
				Obj: &dst.Object{
					Kind: dst.Var,
					Name: "gp",
					Decl: funcDecl.Type.Params.List[0],
				},
			},
			Sel: &dst.Ident{
				Name: "goid",
...
func newInstrumentationStmtOld(prologLoadFuncIdent string, prologCallArgs, epilogCallArgs []dst.Expr) dst.Stmt {
